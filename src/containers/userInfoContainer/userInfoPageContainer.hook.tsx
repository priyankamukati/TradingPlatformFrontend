...
export function UserInfoPageContainerLogic({saveUserInfo, saveUserInfoResponse }: IUserInfoPageContainerProps) {
  ...
  const handleSignUp = async(user_name: string, emailfield: string, password: string, fullName: string, passcode: string) => {
  ...
  const handleSubmit = useCallback(async (event: any) => {
    ...
    if(email && password && fullName && userName && passcode) {
      const user = await handleSignUp(userName, email, password, fullName, passcode);
      if(user) {
  ...
  useEffect(() => {
    if(saveUserInfoResponse.error) {
      // error stuff
    } else if(saveUserInfoResponse.data && saveUserInfoResponse.loading==LoadingState.Idle) {
  ...
  return {
    fullName, userName, passcode, email, password,
    handleOnTickerOnChange, handleCompanyNameChange, handleOnInitialPriceChange, handleSubmit, handleOnEmailChange, handleOnPasswordChange
  }
