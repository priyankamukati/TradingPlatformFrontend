...
export function AdminHomePageContainerLogic({ getAllStocks, saveStock, saveStocksResponse }: IAdminHomePageContainerProps) {
  ...
  const [initialPrice, setInitialPrice] = React.useState<number|undefined>(undefined);
  ...
  const handleSubmit = useCallback((event: any) => {
    ...
    if(ticker && ticker.length > 0 && initialPrice && companyName && initialPrice && volume) {
  ...
  return {
    ticker, companyName, initialPrice, volume,
    handleOnTickerOnChange, handleCompanyNameChange, handleOnInitialPriceChange, handleOnVolumeChange, handleSubmit
  }
