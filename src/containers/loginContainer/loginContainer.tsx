  ...
  defaultProps: Partial<ILoginContainerProps>;
} =({ signOut, user, saveUserInfo, saveUserInfoResponse }: ILoginContainerProps) => {
    ...
    useEffect(() => {
      if(saveUserInfoResponse.error) {
        // error stuff
      } else if(saveUserInfoResponse && saveUserInfoResponse.loading==LoadingState.Idle) {
    ...
    return (
      <div>
      {saveUserInfoResponse.error ? <Alert key={"success"} variant={"danger"}>Error getting user info records</Alert> : <div></div>}
...
export default withAuthenticator(connect<StateToPropsType, DispatchToPropsType>(
  mapStateToProps,
  mapDispatchToProps
)(LoginContainer));
